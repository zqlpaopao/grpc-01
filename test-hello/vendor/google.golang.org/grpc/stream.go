...
type StreamDesc struct {
	StreamName string //流式处理的服务名
	Handler    StreamHandler//流式处理的处理服务地址

	// At least one of these is true.
	ServerStreams bool //服务端流式
	ClientStreams bool//客户端流式
}
...
func (cs *clientStream) SendMsg(m interface{}) (err error) {
	...
	/*
		调用 用 csAttempt 这个结构体中的 sendMsg 方法
	 */
...
func (a *csAttempt) recvMsg(m interface{}, payInfo *payloadInfo) (err error) {
	...
	/*
		recv 再看 recvAndDecompress 方法，调用了 recvMsg
		最终还是调用了 p.r.Read 方法，p.r 是一个 io.Reader 类型。果然万变不离其中，最终都是要落到 IO 上
	 */

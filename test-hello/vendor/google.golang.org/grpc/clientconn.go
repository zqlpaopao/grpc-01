...
func DialContext(ctx context.Context, target string, opts ...DialOption) (conn *ClientConn, err error) {
	...
	/*
		...
		https://github.com/grpc/proposal/blob/master/A14-channelz.md 介绍了channelz的核心内容
	*/
	if channelz.IsOn() {
	...
	/*
		Authentication 这一段是对认证信息的初始化校验
	...
	/*
		Dialer 是发起 rpc 请求的调用器，dialer 中实现了对 rpc 请求调用的具体细节，
	...
	if cc.dopts.copts.Dialer == nil {
		cc.dopts.copts.Dialer = func(ctx context.Context, addr string) (net.Conn, error) {
			...
			/*
				DialContext中 可以看到通过 dialer 的 resolver 来进行服务发现
				这里值得一提的是，通过 dialContext 可以看出，这里的 dial 有两种请求方式，
				一种是 dialParallel ,另一种是 dialSerial。
				dialParallel 发出两个完全相同的请求，采用第一个返回的结果，抛弃掉第二个请求。
				dialSerial 则是发出一串（多个）请求。然后采取第一个返回的请求结果（ 成功或者失败）。
			 */
	...
	scSet := false
	/*
		scChan 是 dialOptions 中的一个属性 scChan  <-chan ServiceConfig
		ServiceConfig 是服务提供方约定的一些参数。这里说明 client 提供给 server 一个可以通过 channel 来修改这些参数的入口。
		client 的某些属性是可以被 server 修改的
	 */
...
//nunaryclientinterceptor将所有的一元客户端拦截器链接为一个。
//对数据进行过滤
